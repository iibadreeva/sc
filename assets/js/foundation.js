...
window.onscroll = function () {
  ...
  scrollMove.init(wScroll);
};

var scrollMove = function () {
  return {
    move: function move(block, windowScroll, strafeAmount) {
      var pos = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'v';
      var strafe = windowScroll / -strafeAmount + 'px',
          style = block.style;
      var transformString;
      if (pos == 'v') transformString = "translateY(".concat(strafe, ")");else transformString = "translateX(".concat(strafe, ")");
      style.transform = transformString;
      style.webkitTransform = transformString;
    },
    init: function init(wScroll) {
      var bg = document.querySelector('.account__bg');
      bg && this.move(bg, wScroll, 2);
    }
  };
}();
...
function textArea() {
  var text_field = $('.form__textarea');
  var text_field_v = text_field.val();
  var cnt_symbol = $('.rem');
  var mod_maxCount = 200;
  cnt_symbol.html(mod_maxCount);
  text_field.keyup(function () {
    var mod_lenText = text_field.val().length;
    var mod_cnt = mod_maxCount - mod_lenText;
    if (mod_lenText >= mod_maxCount) text_field_v = text_field_v.substr(0, mod_maxCount);
    if (mod_cnt <= 0) cnt_symbol.html('0');
    cnt_symbol.html(mod_cnt);
  });
}

function runner() {
  var $runner = $('.js_runner'),
      el;
  $runner.change(function () {
    el = $(this);

    if ($(this).hasClass('js-runner-choice')) {
      el.parent().prev().val(el.val() + "%");
    } else {
      el.parent().prev().val(el.val());
    }
  }).trigger('change');
...
function files_read() {// var filesInput = document.querySelector('.inputFiles');
  // var filesList = document.querySelector('.listFiles');
  // var arr = [];
  //
  ...
  //         arr.push(filesList.appendChild(fileItem));
  //         fileItem.innerHTML = arr[i] + '. ' + '<span>Имя файла: </span>' + $files[i].name;
  //         console.log(arr);
  //     }
  // }, false);
}
...
function () {
  ...
  _createClass(FilesUpload, [{
    key: "onload_files",
    value: function onload_files() {
      $(document).ready(function () {
        var listFiles = $('.listFiles');
        var itemPreviewTemplate = listFiles.find('.itemFile.template').clone();
        var queue = {};
        itemPreviewTemplate.removeClass('template');
        listFiles.find('.itemFile.template').remove();
        ...
        function preview(file) {
          ...
          reader.addEventListener('load', function () {
            ...
            itemPreview.find('span').text(file.name);
            listFiles.append(itemPreview);
        ...
        function delete_file() {
          listFiles.on('click', '.deleteFile', function () {
            var evt_ = new Event("change");
            var itemFile = $(this).closest('.itemFile'); //id = itemFile.data('id');

            delete queue[itemFile];
            itemFile.remove();

            if (document.querySelectorAll('.listFiles .itemFile span').length === 0) {
              document.querySelector('#input_Files').value = '';
              document.querySelector('#input_Files').dispatchEvent(evt_);
            }
          });
        }
      });
...
function create_order_page() {
  try {
    var formCreate = new Form('.form_c');
    formCreate.addEventListeners();
    var formCreate2 = new Form('.form_c2');
    formCreate2.addEventListeners();
    var formCreate3 = new Form('.form_c3');
    formCreate3.addEventListeners();
  } catch (e) {}
  ...
  files_read();
  modalShow.init();
  modalShow.init('.js-modal-link-2', '.js-modal-2');
  new FilesUpload();
}

'use strict'; // function drop_header() {
//     let drop_header = $('.drop_header');
//
//     $(window).scroll(function(){
//         if ($(this).scrollTop() > 500 && drop_header.is(':hidden') ){
//             drop_header.slideDown('fast');
//         } else if($(this).scrollTop() <= 500 && drop_header.is(':visible')) {
//             drop_header.slideUp('fast');
//         }
//     });
// }
...
function auth() {
  ...
  btn_ent.on('click', function () {
    if (log.val().toLowerCase() === '007' && pwd.val() === '12345678') {
      alert('Страница в разработке!');
      wrong_data.hide('fast');
      log.val('').focus();
      pwd.val('');
    } else {
      wrong_data.show('fast');
      log.val('').focus();
      pwd.val('');
    }
  });
  btn_reg.on('click', function () {//document.location.href = 'registration.html';
  });
  log.keypress(function (k) {
    if (k.keyCode === 13) btn_ent.click();
  });
  pwd.keypress(function (k) {
    if (k.keyCode === 13) btn_ent.click();
  });
}
...
function () {
  function Monitor() {
    ...
    this.holder = document.querySelector('.js-slide-wrap');
    this.form = document.querySelector('.js-form');
    this.slideWidth = document.querySelector('.js-slide').offsetWidth;
    this.bindUIEvents();
  }
  ...
  _createClass(Monitor, [{
    ...
    value: function end() {
      ...
      if (this.touchaverage > 99) {
        ...
        var width = this.index === 0 ? 0 : 100; // console.log('end', this.movex, this.slideWidth);
...
var modalShow = function () {
  return {
    init: function init(classLink, classModal) {
      var link = classLink ? document.querySelectorAll(classLink) : document.querySelectorAll('.js-modal-link');
      var modal = document.querySelector(classModal) || document.querySelector('.js-modal');
...
function headerShow() {
  var link = document.querySelector('.js-header-icon'),
      close = document.querySelector('.js-nav-close');
...
function show_pwd() {
  ...
  var inp1 = $('#pwd_reg');
  var inp2 = $('#сnf_pwd');
  var ico_eye1 = $('.ico_eye1');
  var ico_eye2 = $('.ico_eye2');
  btn_pwd1.on('click', function () {
    var change_type1 = inp1.attr('type') === "text" ? "password" : 'text',
        change_ico1 = ico_eye1.toggleClass('fa-eye-slash');
    $(this).append(change_ico1);
    inp1.prop('type', change_type1);
  });
  btn_pwd2.on('click', function () {
    var change_type2 = inp2.attr('type') === "text" ? "password" : 'text',
        change_ico2 = ico_eye2.toggleClass('fa-eye-slash');
    $(this).append(change_ico2);
    inp2.prop('type', change_type2);
  });
}

function menu() {
  var reg_mobile_menu = $('.reg_mobile_menu');
  var reg_mobile_menu_btn = $('.item_menu_reg');
  var reg_close_m_menu = $('.reg_close_m_menu');
  reg_mobile_menu_btn.on('click', function (e) {
    e.preventDefault();
    reg_mobile_menu.toggleClass('reg_m_active');
  });
  reg_close_m_menu.on('click', function (e) {
    e.preventDefault();
    reg_mobile_menu.toggleClass('reg_m_active');
  });
}
...
function slide_text() {
  var open_full = $('.open_full');
  var close_full = $('.close_full');
  var close_full2 = document.querySelector('.close_full i');
  var text_part_2 = $('.part_2');
  close_full.addClass('_hidden');
  open_full.on('click', function () {
    open_full.hide();
    text_part_2.slideToggle(1000);
    text_part_2.css('display', 'flex');
    close_full.removeClass('_hidden');
  ...
  close_full2.addEventListener('click', function () {
    var wScroll = window.pageYOffset;
    scrollTop.grow(wScroll, '.close_full', '.js-scroll-top-form-end', '.js-scroll-top-form');
    text_part_2.slideToggle(1000);
    close_full.addClass('_hidden');
    open_full.show();
  });
...
function check_reg() {
  var reg_user = $('.btnReg_1'); // кнопка регистрации 1 этапа

  var choice_1 = $('.choice_1'); // регистрация 1 этапа

  var choice_2 = $('.choice_2'); // продолжить регистрацию

  var modal_window = $('.modal_window');
  var modal_window_choice = $('.modal_window_choice');
  reg_user.on('click', function () {// sendAjaxRequest(modal_window_choice);
  }); // Модальное окошко первого этапа

  modalShow.init();
  choice_1.on('click', function () {
    modal_window_choice.css('display', 'none');
...
function sendAjaxRequest(modal_window_choice) {
  // CAPTCHA
  ...
  var captcha_mess = $('.form__captcha_mess');

  if (!response) {
    captcha_mess.text('Could not get recaptcha response!');
    return;
  } else {
    captcha_mess.text('');
  ...
  ajax.onreadystatechange = function () {
    if (this.readyState === 4) {
      if (this.status === 200) {
        modal_window_choice.css('display', 'block'); //captcha_mess.text(this.responseText);
      } else if (this.status === 404) {
        captcha_mess.text('Сервер не отвечает!');
      }
    }
  };
...
var scrollTop = function () {
  return {
    grow: function grow(wScroll, startClass, endClass, clickClass) {
      var form = document.querySelector('.second_stage_reg'),
          body = document.querySelector(startClass),
          coord = getCoords(form),
          posCoord = coord.top,
          startAnimate = wScroll - posCoord;
      ...
      this.click(endClass, clickClass);
    },
    click: function click(endClass, clickClass) {
      var scrollTop = document.querySelector(clickClass);
      scrollTop.addEventListener('click', function () {
        var formSend = document.querySelector(endClass),
...
function scrollForm() {
  window.onscroll = function () {
    var wScroll = window.pageYOffset;
    scrollTop.grow(wScroll, '.js-scroll-top', '.js-send', '.js-scroll-top');
  };
}
...
function registration_init() {
  ...
  scrollForm();
} // window.addEventListener("load", formTwo);
...
function () {
  function Form(className) {
    ...
    var formTwo = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  ...
  _createClass(Form, [{
    ...
    value: function addEventListeners() {
      ...
      for (var i = 0; i < this.input.length; i++) {
        if (this.input[i].getAttribute('type') === 'checkbox') {
          this.input[i].addEventListener('change', this.validate.bind(this));
        } else if (this.input[i].getAttribute('type') === 'file') {
          this.input[i].addEventListener('change', this.validate.bind(this));
        } else {
          this.input[i].addEventListener('blur', this.validate.bind(this));
          this.input[i].addEventListener('keyup', this.validate.bind(this));
          this.input[i].addEventListener('blur', this.errorValid.bind(this));
          this.input[i].addEventListener('keyup', this.errorValid.bind(this));
        }
      }

      for (var _i = 0; _i < this.select.length; _i++) {
        for (var j = 0; j < this.select[_i].querySelectorAll('ul li').length; j++) {
          this.select[_i].querySelectorAll('ul li')[j].addEventListener('click', function () {
            setTimeout(function () {
              _this.validate();
            }, 200);
          });
    ...
    key: "errorValid",
    value: function errorValid(event) {
      var error = event.target.parentNode.querySelector('.form_reg_two__error');
      var errorText = 'Значение не должно быть пустое';

      if (event.target.dataset.ignore === 'true') {//
      } else if (event.target.dataset.type === 'password') {
        if (event.target.parentNode.classList.contains('js-pass-1')) {
          var input = event.target.parentNode.nextSibling.nextSibling.querySelector('input');
          var err = event.target.parentNode.nextSibling.nextSibling.querySelector('.form_reg_two__error');

          if (event.target.dataset.text && event.target.value.length < 5 || input.value !== event.target.value) {
            errorText = event.target.value.length < 5 ? 'Пароль должен быть не мение 5 символов' : 'Пароли не совпадают';
            event.target.dataset.text = errorText;
            error.textContent = event.target.dataset.text;
          } else {
            error.textContent = '';
            err.textContent = '';
          }
        } else if (event.target.parentNode.classList.contains('js-pass-2')) {
          var _input = event.target.parentNode.previousSibling.previousSibling.querySelector('input');

          var _err = event.target.parentNode.previousSibling.previousSibling.querySelector('.form_reg_two__error');

          if (event.target.dataset.text && event.target.value.length < 5 || _input.value !== event.target.value) {
            errorText = event.target.value.length < 5 ? 'Пароль должен быть не мение 5 символов' : 'Пароли не совпадают';
            event.target.dataset.text = errorText;
            error.textContent = event.target.dataset.text;
          } else {
        ...
        if (event.target.parentNode.classList.contains('js-email')) {
          if (event.target.value.match(/^[\w-\.]+@[\w-]+\.[a-z]{2,3}$/i)) {
            error.textContent = '';
          } else {
            error.textContent = event.target.dataset.text;
          }
        }
      } else if (event.target.getAttribute('type') == 'range') {} else if (event.target.getAttribute('id') == 'login_reg') {
        if (event.target.dataset.text && event.target.value.length < 5) {
          error.textContent = event.target.dataset.text;
        } else {
          error.textContent = '';
        }
      } else {
        if (event.target.dataset.text && event.target.value === '') {
          error.textContent = event.target.dataset.text;
        } else {
          error.textContent = '';
        }
    ...
    key: "errorValidSelect",
    value: function errorValidSelect(event) {
      var error = event.target.parentNode.parentNode.parentNode.querySelector('.form_reg_two__error-select');
      var select = event.target.parentNode.parentNode.parentNode.querySelector('select');
      var errorText = event.target.parentNode.parentNode.parentNode.querySelector('select').dataset.text;

      if (select.dataset.ignore === 'true') {//
      } else {
        if (errorText && event.target.dataset.value === '-') {
    ...
    value: function validate(event) {
      ...
      for (var i = 0; i < this.input.length; i++) {
        if (this.input[i].dataset.ignore === 'true') {
          matchList.push(1);
        } else if (this.input[i].getAttribute('type') === 'checkbox') {
          if (this.input[i].checked === true) {
            matchList.push(1);
          } else {
            matchList.push(0);
          }
        } else if (this.input[i].getAttribute('type') === 'email') {
          if (this.input[i].value !== '' && this.input[i].value.match(/^[\w-\.]+@[\w-]+\.[a-z]{2,3}$/i)) {
            matchList.push(1);
          } else {
            matchList.push(0);
          }
        } else if (this.input[i].getAttribute('id') == 'login_reg') {
          if (this.input[i].value.length >= 5) {
            matchList.push(1);
          } else {
            matchList.push(0);
          }
        } else if (this.input[i].dataset.type === 'password') {
          if (this.input[i].parentNode.classList.contains('js-pass-1')) {
            var input = this.input[i].parentNode.nextSibling.nextSibling.querySelector('input');

            if (this.input[i].value.length >= 5 && input.value === this.input[i].value) {
              matchList.push(1);
            } else {
              matchList.push(0);
            }
          } else if (this.input[i].parentNode.classList.contains('js-pass-2')) {
            var _input2 = this.input[i].parentNode.previousSibling.previousSibling.querySelector('input');

            if (this.input[i].value.length >= 5 && _input2.value === this.input[i].value) {
              matchList.push(1);
            } else {
              matchList.push(0);
            }
      ...
      var formTwoActive = true;

      if (this.formTwo) {
        var _formTwo = document.querySelector(this.formTwo);

        var formTwoButton = _formTwo.querySelector('button');
      ...
      var isValid = matchList.find(function (item) {
        return item === 0;
      });
      var isValidSelect = matchSelect.find(function (item) {
        return item === 0;
      });
    ...
    value: function file_reading() {
      ...
      var _loop = function _loop(i) {
        ...
        fileReading.addEventListener('load', function () {
          if (imgage.classList.contains('change')) {
            imgage.src = this.result;
            imgage.classList.remove('change');
            fields_image.classList.add('active');
          }
        });
        photoInput[i].addEventListener('change', function (e) {
          var file = e.target.files[0];

          if (file) {
            photoInput[i].parentNode.querySelector('img').classList.add('change');
            photoInput[i].parentNode.querySelector('label').style.display = 'none';
            fileReading.readAsDataURL(file);
          }
        });
        close.addEventListener('click', function () {
          var event = new Event("change");
          fields_image.classList.remove('active');
          photoInput[i].parentNode.querySelector('label').style.display = 'block';
          imgage.src = '';
          photoInput[i].value = '';
          photoInput[i].dispatchEvent(event);
        });
      };
    ...
    key: "files_reading",
    value: function files_reading() {
      document.getElementById('photo_pass').addEventListener('change', handleFileSelect, false);
    }
  }]);
  ...
  for (var i = 0, f; f = files[i]; i++) {
    ...
    reader.onload = function (theFile) {
      return function (e) {
        ...
        span.classList.add('form__image-group__image');
        span.classList.add('active');
        span.innerHTML = ['<img class="thumb" src="', e.target.result, '" title="', escape(theFile.name), '"/><span class="close"></span>'].join('');
        document.getElementById('list').insertBefore(span, null);
        evt.target.parentNode.querySelector('label').style.display = 'none';
        handleFileClose();
      };
    }(f);
...
function handleFileClose() {
  ...
  var _loop2 = function _loop2(i) {
    ...
    close.addEventListener('click', function () {
      var event = new Event("change");
      document.querySelector('#list').removeChild(item[i]);

      if (document.querySelectorAll('#list .form__image-group__image').length === 0) {
        document.querySelector('.form__image-group__label').style.display = 'block';
        document.querySelector('#photo_pass').value = '';
        document.querySelector('#photo_pass').dispatchEvent(event);
      }
    });
  };
...
(function () {
  ...
  24: [function (_dereq_, module, exports) {
    ...
    var TAG = _dereq_(125)('toStringTag'); // ES3 wrong here
  ...
  36: [function (_dereq_, module, exports) {
    ...
    var document = _dereq_(45).document; // typeof document.createElement is 'object' in old IE
  ...
  49: [function (_dereq_, module, exports) {
    module.exports = !_dereq_(35) && !_dereq_(41)(function () {
      return Object.defineProperty(_dereq_(36)('div'), 'a', {
        get: function get() {
          return 7;
        }
      }).a != 7;
  ...
  52: [function (_dereq_, module, exports) {
    ...
    var cof = _dereq_(25); // eslint-disable-next-line no-prototype-builtins
  ...
  102: [function (_dereq_, module, exports) {
    ...
    var defined = _dereq_(34); // true  -> String#at
  ...
  116: [function (_dereq_, module, exports) {
    ...
    var isObject = _dereq_(56); // instead of the ES6 spec version, we didn't implement @@toPrimitive case
  ...
  134: [function (_dereq_, module, exports) {
    ...
    $export($export.S + $export.F * !_dereq_(61)(function (iter) {
      Array.from(iter);
    }), 'Array', {
  ...
  137: [function (_dereq_, module, exports) {
    ...
    var toIObject = _dereq_(113); // 22.1.3.4 Array.prototype.entries()
  ...
  141: [function (_dereq_, module, exports) {
    ...
    var createProperty = _dereq_(30); // WebKit Array.of isn't generic
  ...
  149: [function (_dereq_, module, exports) {
    ...
    var toISOString = _dereq_(32); // PhantomJS / old WebKit has a broken implementations
  ...
  154: [function (_dereq_, module, exports) {
    ...
    if (!(HAS_INSTANCE in FunctionProto)) _dereq_(74).f(FunctionProto, HAS_INSTANCE, {
  ...
  174: [function (_dereq_, module, exports) {
    ...
    if (!$Number(' 0o1') || !$Number('0b1') || $Number('+0x1')) {
      ...
      for (var keys = _dereq_(35) ? gOPN(Base) : ( // ES3:
  ...
  182: [function (_dereq_, module, exports) {
    ...
    var $parseFloat = _dereq_(87); // 20.1.2.12 Number.parseFloat(string)
  ...
  183: [function (_dereq_, module, exports) {
    ...
    var $parseInt = _dereq_(88); // 20.1.2.13 Number.parseInt(string, radix)
  ...
  184: [function (_dereq_, module, exports) {
    ...
    $export($export.P + $export.F * (!!$toFixed && (0.00008.toFixed(3) !== '0.000' || 0.9.toFixed(0) !== '1' || 1.255.toFixed(2) !== '1.25' || 1000000000000000128.0.toFixed(0) !== '1000000000000000128') || !_dereq_(41)(function () {
      // V8 ~ Android 4.3-
      $toFixed.call({});
    })), 'Number', {
  ...
  187: [function (_dereq_, module, exports) {
    var $export = _dereq_(39); // 19.1.2.2 / 15.2.3.5 Object.create(O [, Properties])
  ...
  188: [function (_dereq_, module, exports) {
    var $export = _dereq_(39); // 19.1.2.3 / 15.2.3.7 Object.defineProperties(O, Properties)
  ...
  189: [function (_dereq_, module, exports) {
    var $export = _dereq_(39); // 19.1.2.4 / 15.2.3.6 Object.defineProperty(O, P, Attributes)
  ...
  203: [function (_dereq_, module, exports) {
    ...
    var $parseFloat = _dereq_(87); // 18.2.4 parseFloat(string)
  ...
  204: [function (_dereq_, module, exports) {
    ...
    var $parseInt = _dereq_(88); // 18.2.5 parseInt(string, radix)
  ...
  205: [function (_dereq_, module, exports) {
    ...
    Wrapper = _dereq_(29)[PROMISE]; // statics
  ...
  207: [function (_dereq_, module, exports) {
    ...
    var rConstruct = (_dereq_(45).Reflect || {}).construct; // MS Edge supports only 2 arguments and argumentsList argument is optional
  ...
  208: [function (_dereq_, module, exports) {
    ...
    var toPrimitive = _dereq_(116); // MS Edge has broken Reflect.defineProperty - throwing instead of returning false
  ...
  220: [function (_dereq_, module, exports) {
    ...
    if (_dereq_(35) && (!CORRECT_NEW || _dereq_(41)(function () {
      re2[_dereq_(125)('match')] = false; // RegExp constructor can alter flags and IsRegExp works correct with @@match
  ...
  240: [function (_dereq_, module, exports) {
    ...
    var $at = _dereq_(102)(true); // 21.1.3.27 String.prototype[@@iterator]()
  ...
  250: [function (_dereq_, module, exports) {
    ...
    $Symbol[PROTOTYPE][TO_PRIMITIVE] || _dereq_(47)($Symbol[PROTOTYPE], TO_PRIMITIVE, $Symbol[PROTOTYPE].valueOf); // 19.4.3.5 Symbol.prototype[@@toStringTag]
  ...
  262: [function (_dereq_, module, exports) {
    ...
    var $WeakMap = module.exports = _dereq_(28)(WEAK_MAP, wrapper, methods, weak, true, true); // IE11 WeakMap frozen keys fix
  ...
  269: [function (_dereq_, module, exports) {
    ...
    var userAgent = _dereq_(121); // https://github.com/zloirock/core-js/issues/280
...
(function () {
  ...
  (function (e) {
    ...
    function t(t) {
      return typeof t == "string" ? e.document.createTextNode(t) : t;
    }

    function n(n) {
      if (n.length === 1) return t(n[0]);

      for (var r = e.document.createDocumentFragment(), i = v.call(n), s = 0; s < n.length; s++) {
        r.appendChild(t(i[s]));
      }
    ...
    for (var r = Object.defineProperty || function (e, t, n) {
      ...
      return n;
    }, s, o, u, a, f = /^\s+|\s+$/g, l = /\s+/, c = " ", h = function h(t, n) {
      if (this.contains(t)) n || this.remove(t);else if (n === undefined || n) n = !0, this.add(t);
      return !!n;
    }, p = (e.Element || e.Node || e.HTMLElement).prototype, d = ["matches", p.matchesSelector || p.webkitMatchesSelector || p.khtmlMatchesSelector || p.mozMatchesSelector || p.msMatchesSelector || p.oMatchesSelector || function (t) {
      var n = this.parentNode;
      return !!n && -1 < i.call(n.querySelectorAll(t), this);
    }, "closest", function (t) {
      var n = this,
          r;
    ...
    }], v = d.slice, m = d.length; m; m -= 2) {
      o = d[m - 2], o in p || (p[o] = d[m - 1]);
    }

    document.createElement("a").matches("a") || (p[o] = function (e) {
      return function (t) {
        return e.call(this.parentNode ? this : document.createDocumentFragment().appendChild(this), t);
      };
    }(p[o])), "classList" in document.documentElement ? (a = document.createElement("div").classList, a.add("a", "b", "a"), "a b" != a && (p = a.constructor.prototype, "add" in p || (p = e.DOMTokenList.prototype), u = function u(e) {
      return function () {
        var t = 0;
    ...
    }, p.add = u(p.add), p.remove = u(p.remove), p.toggle = h)) : (u = function u(e) {
      if (!e) throw "SyntaxError";
      if (l.test(e)) throw "InvalidCharacterError";
      return e;
    }, a = function a(e) {
      var t = e.className.replace(f, "");
      t.length && d.push.apply(this, t.split(l)), this._ = e;
    }, a.prototype = {
      length: 0,
      add: function add() {
        for (var t = 0, n; t < arguments.length; t++) {
          n = arguments[t], this.contains(n) || d.push.call(this, o);
        }
      ...
      contains: function (e) {
        return function (n) {
          return m = e.call(this, o = u(n)), -1 < m;
        };
      }([].indexOf || function (e) {
        m = this.length;
      ...
      remove: function remove() {
        for (var t = 0, n; t < arguments.length; t++) {
          n = arguments[t], this.contains(n) && d.splice.call(this, m, 1);
        }
      ...
      set: function set() {}
    })), "head" in document || r(document, "head", {
      get: function get() {
        return s || (s = document.getElementsByTagName("head")[0]);
      }
    });
...
}).call((typeof window === "undefined" ? "undefined" : _typeof(window)) === 'object' && window || (typeof self === "undefined" ? "undefined" : _typeof(self)) === 'object' && self || (typeof global === "undefined" ? "undefined" : _typeof(global)) === 'object' && global || {});